...
def indexView(request):
    accessories = Product.objects.filter(cat_name__id=9)
    smartphone = Product.objects.filter(cat_name__id=10)
    laptop = Product.objects.filter(cat_name__id=11)
    prod = Product.objects.all()
    # Pagination Featur fruit
    paginator = Paginator(accessories, 6, orphans=0)
    page_number = request.GET.get('page')
    accessories = paginator.get_page(page_number)

    # Pagination smartphone
    paginator = Paginator(smartphone, 6, orphans=0)
    page_number = request.GET.get('page')
    smartphone = paginator.get_page(page_number)

    # Pagination laptop
    paginator = Paginator(laptop, 6, orphans=0)
    page_number = request.GET.get('page')
    laptop = paginator.get_page(page_number)

    return render(request, 'index.html', {'accessories': accessories, 'smartphone': smartphone, 'laptop': laptop, 'page_number': page_number, 'prod': prod})
    ...
    if 'user' in request.session:
        ...
        if request.method == "POST":
            ...
            data_dict = {
                ...
                'MOBILE_NO': '9624889080',
                'EMAIL': 'ranipanirmal@gmail.com',
                'CUST_ID': '123123',
                'ORDER_ID': order_id,
